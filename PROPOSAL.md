...
**EX — Examples presence.** _Inputs:_ `guide.patterns()` (every ```ts fenced code block's body text, whole document) vs `source.examples()` / `source.examples(name)` (exported functions, or a named interface/class's members, whose immediately preceding JSDoc carries `@example`). _Algorithm:_ a documented symbol "has an example" when its bare name appears at a word boundary in ANY Patterns fence, OR the symbol is a member of the `@example`-carrying set — `findUnexampled(names, fences, examples)` returns the names satisfying neither. Applies to every `function`-kind `Surface` symbol (`source.examples()`) and every `MethodGroup` member (`source.examples(group.interface).concat(source.examples(entity))`, unioning the interface's and the by-convention implementer's own `@example`-carrying members). Presence-only: fence and JSDoc CONTENT are never checked, only that one of the two forms exists. _Guard:_ reuses the SB/MB extractions, already proven non-vacuous. _Failing diff:_ `[ 'farewell' ]` (a documented function with no fence mention and no `@example` JSDoc).

**FI — Fence-import reality.** _Inputs:_ every `import { ... } from 'specifier'` statement in each `guide.patterns()` fence, parsed by `fenceImports` into `{ specifier, names }` (handling `import type`, mixed/multiline braces, and `x as y` aliases, resolved to the ORIGINAL exported name). _Algorithm:_ for every fence import whose `specifier` is one of the consumer's own SELF specifiers (this repo's published package name and/or its `@src/core`-style path alias), `findMissing(names, source.exports().map(name))` must be empty — a fence must never import a name the module does not actually export. A non-self specifier (an external dependency) is skipped; FI checks only the documented package's own surface, not third-party imports. _Guard:_ none needed beyond the fence's own presence — an empty-fence guide trivially passes (vacuously true, same as an empty manifest failing NV separately). _Failing diff:_ `[ 'ghost' ]` (a fence imports a name the module never exports).
...
**Future work (post-v1):** the TS-compiler-API `Source` for cross-language/non-conforming repos; backtick-prose resolution (every prose backtick resolves to an export, a member, an attributed external, or a language literal); `## Patterns` fence typechecking; a tests-mirror sub-check (every behavioral source file has a linked test); mirror-identity checking for a dependency mirror guide (e.g. `markdown.md` in this repo) against the ACTUAL upstream package's surface — deferred because it requires the upstream repo to ship its own `guides/` inside its published npm tarball, which cross-repo mirrors currently do not (a consumer's `node_modules` inventory has no `guides/` to reflect against, only compiled `dist/`).
