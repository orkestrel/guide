...
| `extractDeclaration`    | function | `(source: string, keyword: 'class' \| 'interface', name: string) => Declaration \| undefined`             | Locates one real `export class` / `export interface` head in projected lines and returns its raw body and its `extends` bases together, or `undefined` when the file declares no such head.                                                       |
